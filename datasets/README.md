...
byte-identical matrix parity copies retained only for final integrity and single-copy cleanup.
Maintained examples no longer read them. Synthetic data are generated at runtime and are not
committed as dataset files. No generic loader or registry is provided.
...
During the final transition stage, these directories are parity fixtures only. `metadata.yaml`
documents each temporary asset but is not a runtime model input. Full source citations and
preparation details are given in each dataset README and metadata file.
