...
caller-owned Matplotlib rendering remains the plotting boundary. Patches 1–6 are complete:
...
attributes have been replaced by `search.select(rule="best_score")`; `cv_results_` now uses only
stable `n_components` and `predictor_rank` parameter columns; `PiPLSDataset` uses only `X` and `Y`;
and unused shape-only inspection properties are removed while component counts remain. Top-level
result-export reduction and final active-surface synchronization are the next implementation
increment.
