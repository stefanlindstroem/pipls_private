...
\definecolor{predictoraccent}{HTML}{5A67A8}
\definecolor{sharedaccent}{HTML}{2E887A}
\definecolor{responseaccent}{HTML}{C4823B}
...
\begin{tikzpicture}[node distance=1.35cm and 1.65cm]
  ...
  \draw[signal, draw=predictoraccent]
    ...
    node[edge label, pos=0.52, left] {$\mathbf{L}_{\mathrm{p}}$} (x.north west);

  \draw[signal, draw=sharedaccent]
    (shared.south west) to[bend right=9]
    node[edge label, pos=0.52, right] {$\mathbf{L}_{\mathrm{sp}}$} (x.north east);

  \draw[signal, draw=sharedaccent]
    (shared.south east) to[bend left=9]
    node[edge label, pos=0.52, left] {$\mathbf{L}_{\mathrm{sr}}$} (y.north west);

  \draw[signal, draw=responseaccent]
    (resp.south) to[bend left=8]
    node[edge label, pos=0.52, right] {$\mathbf{L}_{\mathrm{r}}$} (y.north east);
