"""Write the Pulp Pi-PLS component path as a focused CSV smoke check."""
...
def _result_rows(search: PiPLSPathCV) -> list[dict[str, ResultValue]]:
    ...
    for index in range(len(results["n_components"])):
        ...
        if row["predictor_rank_policy"] != "optimized":
            raise RuntimeError("The public Pulp path must conditionally optimize predictor rank.")
...
def run_benchmark() -> list[dict[str, ResultValue]]:
    """Run the Pulp component-path smoke check."""
