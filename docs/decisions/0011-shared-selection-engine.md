...
Refined and implemented by Decision 0039. `PiPLSSearchCV` is the sole public selection interface;
...
`(n_components, predictor_rank)` model. `PiPLSSearchCV` remains a meta-estimator that
...
- `PiPLSSearchCV` uses those components for each row of the triangular path.
...
- `PiPLSSearchCV` continues to support complete pipelines without creating a circular
  public dependency;
