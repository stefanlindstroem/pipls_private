...
- `metadata.yaml` follows the versioned repository contract in `.llm/dataset_layout.md` and records
  descriptions, source, licensing, dimensions, variables, alignment, missingness, preparation, and
  integrity hashes.
...
- Repository reproducibility is strengthened without broadening the public API.
- Existing real-dataset integrations must be normalized to this layout when the convention is
  adopted.
