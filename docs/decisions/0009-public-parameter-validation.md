...
- `n_jobs` must be `None` or a nonzero Python or NumPy integer. Negative joblib values remain
  valid.
- `random_state` must be in the unsigned 32-bit seed interval
  $[0, 2^{32}-1]$. `None` is accepted only for `svd_solver="full"`; `"auto"` may select
  randomized SVD and therefore requires a valid seed.
