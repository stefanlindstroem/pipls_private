...
`select()` resolves one stored row without fitting. `refit()` resolves the same row, clones the
configured estimator or pipeline, fits it on all supplied data, and attaches the exact immutable
selection as `model.selection_`. `oof_report()` accepts an existing compatible selection, refits its
